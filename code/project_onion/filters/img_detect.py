...
names = path.abspath(path.join(__file__ ,"../../YOLOv3/project_onion_yolo.names"))
weights = path.abspath(path.join(__file__ ,"../../YOLOv3/project_onion_yolo.weights"))
cfg = path.abspath(path.join(__file__ ,"../../YOLOv3/project_onion_yolo.cfg"))
...
def detect_object(fs, filename):
    ...
    try:
        ...
        if image is None:
            print("Error: Failed to load image from GridFS.")
            return False
