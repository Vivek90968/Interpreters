...
def word_cloud(request):
    ...
    if len(iterativeCrawledKeywords)>0:
        ...
        return render(request, 'dashboard/wordclouddash.html', {'title':"keywords", 'form':form, 'wc': True})
    return render(request, 'dashboard/wordclouddash.html', {'title':"keywords"})
        ...
        if request.method =='POST':
            ...
            if select_platform_dropdown.is_valid():
                ...
                if select_keyword_dropdown.is_valid():
                    ...
                    result = dash_selected.display_link_similarity(selected_database, selected_collections)
                    if isinstance(result, bool):
                        print("Error occurred while retrieving link similarity data")
                    else:
                        try:
                            links, matrix, no_of_links, percentages, all_count = result
                        except ValueError:
                              print("Unexpected format of the result")
                    links_matrix = zip(links, matrix, percentages)
    ...
    if request.method =='POST':
        ...
        if select_link_dropdown.is_valid():
            ...
            if selected_link == "All links":
                ...
                result = dash.get_activity_period(selected_link)
                if result is not None:
                   activity, percentage_activity, custom_activity_matrix = result
                else:
                     activity = []
                     percentage_activity = 0
