...
class Dashboard:
    ...
    def active_inactive(self, database, collection):
        if database is None or collection is None:
            return False
        coll = connect_mongodb(database, collection)
        active_links = coll.count_documents({"Link status": "Active"})
        inactive_links = coll.count_documents({"Link status": "Inactive"})        
    ...
    def get_visited_keywords(self, platform_choice, link_or_content):
        ...
        for x in coll.find():
            ...
            links_images = []
            for x in coll.find():
             if "Link" in x and "Images" in x:
                links_images.append([x["Link"], x["Images"]])

    # Assuming the field variable contains the key to access the keyword
             if "Keyword" in x:
    ...
    def get_activity_period(self, link):
        ...
        custom_activity_document = coll.find_one({"Link": link})
        custom_activity = custom_activity_document.get("Custom", [])
        custom_activity_matrix = []

        if isinstance(custom_activity, list) and custom_activity:
            custom_activity = custom_activity[0]
