...
def img_processing(request):
    ...
    result = get_images(database, collection)
    
    # Check if the result is False
    if result is False:
        messages.error(request, 'Failed to retrieve images from the database.')
        return render(request, 'filters/img_process.html', {'related_links': [], 'no_of_links': 0, 'title': "Image Processing"})
    
    coll, links_images, coll_fs_files = result
